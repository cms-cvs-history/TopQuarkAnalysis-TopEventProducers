...
def cloneTtFullHadEvent(process
    ...
    ## set the correct name for the hypothesis as *cloneProcessingSnippet*
    ## can only rename InputTags automatically and not strings (being 
    ## misused here as InputTags ...)
    for obj in range(len(getattr(process,"ttFullHadEvent"+str(i)).hypotheses)):
        label = getattr(process,"ttFullHadEvent"+str(i)).hypotheses[0]
        getattr(process,"ttFullHadEvent"+str(i)).hypotheses.remove(label)
        label = label+str(i)
        getattr(process,"ttFullHadEvent"+str(i)).hypotheses.append(label)
